...
final class MiscTests: MemoryRendererTestCase {
    ...
    func testForSugar() throws {
        ...
        """
        try XCTAssertEqual(render(raw: template, ["names": ["Vapor", "Template", "Bits"]]), expect)
    }
    ...
    func testSmallBugs() throws {
        struct TakesNilParam: Function, Invariant, BoolReturn {
            static var callSignature: [CallParameter] {[.string(labeled: nil, optional: true)]}
            func evaluate(_ params: CallValues) -> TemplateData {
                .bool(params[0].string == nil)
            }
        }
        
        TemplateConfiguration.entities.use(TakesNilParam(), asFunction: "takesNil")
